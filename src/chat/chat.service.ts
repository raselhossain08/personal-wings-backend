...
export class ChatService {
  ...
  async getUserConversations(userId: string, page: number = 1, limit: number = 20): Promise<{ conversations: Conversation[]; total: number }> {
  ...
  async sendMessage(conversationId: string, createMessageDto: CreateMessageDto, userId: string): Promise<Message> {
  ...
  async getMessages(conversationId: string, userId: string, page: number = 1, limit: number = 50): Promise<{ messages: Message[]; total: number }> {
    const canAccess = await this.canUserAccessConversation(conversationId, userId);
    if (!canAccess) {
      throw new ForbiddenException('Cannot access this conversation');
    }
