...
export class PaymentsService {
  ...
  async createPaymentIntent(createPaymentIntentDto: CreatePaymentIntentDto, userId: string) {
    const { amount, currency, paymentMethod, courseIds, description } = createPaymentIntentDto;
    ...
    switch (paymentMethod) {
      case PaymentMethod.STRIPE:
        paymentResult = await this.stripeService.createPaymentIntent(amount, currency, metadata);
        break;
      
      case PaymentMethod.PAYPAL:
        const items = courseIds ? await this.getCourseItems(courseIds) : [];
        paymentResult = await this.paypalService.createOrder(amount, currency, items);
        break;
    ...
    await this.createTransaction({
      ...
      gatewayTransactionId: paymentResult.paymentIntentId || paymentResult.orderId,
      orderId: order.id,
    });
  ...
  async processPayment(processPaymentDto: ProcessPaymentDto, userId: string) {
    ...
    try {
      switch (paymentMethod) {
        case PaymentMethod.STRIPE:
          paymentResult = await this.stripeService.confirmPayment(paymentIntentId);
          break;
        
        case PaymentMethod.PAYPAL:
          paymentResult = await this.paypalService.captureOrder(paymentIntentId);
          break;
  ...
  async updateTransaction(gatewayTransactionId: string, status: TransactionStatus, updates: any = {}): Promise<Transaction> {
  ...
  async getUserTransactions(userId: string, page: number = 1, limit: number = 10): Promise<{ transactions: Transaction[], total: number }> {
  ...
  async handleStripeWebhook(payload: any, signature: string) {
    const { event, type } = await this.stripeService.handleWebhook(payload, signature);
  ...
  async requestRefund(orderId: string, reason: string, userId: string) {
    ...
    try {
      if (order.paymentMethod === PaymentMethod.STRIPE) {
        refundResult = await this.stripeService.refundPayment(order.paymentIntentId);
      } else {
        throw new BadRequestException('Refunds for this payment method are not yet supported');
      }
  ...
  private async handlePaymentSuccess(paymentIntent: any) {
    const order = await this.orderModel.findOne({ paymentIntentId: paymentIntent.id });
    if (order) {
      order.status = OrderStatus.COMPLETED;
      order.paidAt = new Date();
      await order.save();
  ...
  private async handlePayPalPaymentSuccess(payload: any) {
    const order = await this.orderModel.findOne({ paymentIntentId: payload.resource.id });
    if (order) {
      order.status = OrderStatus.COMPLETED;
      order.paidAt = new Date();
      await order.save();
  ...
  private async handlePayPalPaymentFailure(payload: any) {
    ...
    if (order) {
      ...
      await this.updateTransaction(
        payload.resource.id,
        TransactionStatus.FAILED,
        { failureReason: payload.resource.status_details?.reason || 'Payment failed' }
      );
    }
  }
}