...
export class Transaction extends Document {
  ...
  @ApiProperty({ enum: TransactionType, example: TransactionType.PAYMENT, description: 'Transaction type' })
  @Prop({ type: String, enum: TransactionType, required: true })
  type: TransactionType;

  @ApiProperty({ enum: TransactionStatus, example: TransactionStatus.COMPLETED, description: 'Transaction status' })
  ...
  @ApiProperty({ example: 'ord_123', description: 'Related order ID', required: false })
  @Prop()
  orderId: string;
  ...
  @ApiProperty({ example: 'Customer requested refund', description: 'Failure reason', required: false })
  @Prop()
