...
export class PaymentsController {
  ...
  async createPaymentIntent(@Body() createPaymentIntentDto: CreatePaymentIntentDto, @Req() req) {
  ...
  async processPayment(@Body() processPaymentDto: ProcessPaymentDto, @Req() req) {
  ...
  async handleStripeWebhook(@Body() body: any, @Req() req) {
    return this.paymentsService.handleStripeWebhook(body, req.headers['stripe-signature']);
  }
  ...
  async requestRefund(@Param('orderId') orderId: string, @Body() body: { reason: string }, @Req() req) {
    return this.paymentsService.requestRefund(orderId, body.reason, req.user.id);
  }
}