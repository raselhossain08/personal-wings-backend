...
export class SupportController {
  ...
  @Get('my-tickets')
  getUserTickets(@Request() req, @Query('page') page?: number, @Query('limit') limit?: number) {
  ...
  addReply(
    ...
    const isStaff = [UserRole.ADMIN, UserRole.SUPER_ADMIN, UserRole.INSTRUCTOR].includes(req.user.role);
    return this.supportService.addReply(id, createReplyDto, req.user.userId, isStaff);
  }

  @Post(':id/rate')
  rate(@Param('id') id: string, @Body() rateTicketDto: RateTicketDto, @Request() req) {
