...
export class SupportService {
  ...
  async findAll(filters: any): Promise<any> {
    const { page = 1, limit = 20, status, category, priority, assignedTo, userId } = filters;
    const query: any = {};
  ...
  async addReply(ticketId: string, createReplyDto: CreateReplyDto, userId: string, isStaff: boolean): Promise<TicketReply> {
  ...
  async rateTicket(id: string, rateTicketDto: RateTicketDto, userId: string): Promise<Ticket | null> {
    const ticket = await this.ticketModel.findById(id).exec();
    if (!ticket) {
      throw new NotFoundException('Ticket not found');
    }
  ...
  async getTicketStats(): Promise<any> {
    ...
    const avgResolutionTime = await this.ticketModel.aggregate([
      {
        $match: {
    ...
    const satisfactionRating = await this.ticketModel.aggregate([
