...
import { SendMessageDto, CreateKnowledgeDto, RateBotDto, EscalateToHumanDto } from './dto/ai-bot.dto';
import { CreateBotTaskDto, UpdateBotTaskDto, AssignTaskDto, BulkAssignTaskDto } from './dto/bot-task.dto';
...
export class AiBotController {
  ...
  escalateToHuman(@Body() escalateDto: EscalateToHumanDto, @Request() req) {
    return this.aiBotService.escalateToHuman(escalateDto.sessionId, escalateDto.reason || 'user_request', req.user.userId);
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.ADMIN, UserRole.SUPER_ADMIN)
  updateKnowledge(@Param('id') id: string, @Body() updates: Partial<CreateKnowledgeDto>) {
  ...
  bulkAssignTasks(@Body() bulkAssignDto: BulkAssignTaskDto) {
    return this.aiBotService.bulkAssignTasks(bulkAssignDto.taskIds, bulkAssignDto.assignedTo);
  }
