...
4. Always be positive and solution-oriented`;
  ...
  async detectIntentWithGPT(message: string, conversationHistory: any[]): Promise<{
    intent: BotIntent;
    confidence: number;
    reasoning?: string;
  }> {
  ...
  async generateResponseWithGPT(
    ...
    if (!this.openai) {
      return {
        message: "I'm having trouble connecting to my AI brain right now. Let me connect you with a human agent.",
    ...
    try {
      ...
      if (!responseText) {
        return {
          message: 'I apologize, but I encountered an error. Would you like me to connect you with a human agent?',
  ...
  async answerWithGPT(
    ...
    try {
      ...
      return completion.choices[0].message.content || 'I apologize, but I encountered an error processing your question.';
    } catch (error) {
  ...
  private generateQuickReplies(intent: BotIntent): string[] {
    const quickReplies: Record<string, string[]> = {
      [BotIntent.GREETING]: ['Browse courses', 'My enrollments', 'Help center', 'Talk to human'],
      [BotIntent.COURSE_INQUIRY]: ['Show courses', 'Course categories', 'Popular courses', 'Pricing'],
      [BotIntent.ENROLLMENT_HELP]: ['How to enroll', 'Payment options', 'Course requirements'],
      [BotIntent.PAYMENT_ISSUE]: ['Retry payment', 'View orders', 'Payment methods', 'Get help'],
      [BotIntent.REFUND_REQUEST]: ['Request refund', 'Refund policy', 'Check status', 'Talk to human'],
      [BotIntent.CERTIFICATE_INQUIRY]: ['Check progress', 'View certificates', 'Requirements'],
      [BotIntent.TECHNICAL_SUPPORT]: ['Troubleshooting', 'Report issue', 'Talk to technician'],
      [BotIntent.ACCOUNT_HELP]: ['Reset password', 'Update profile', 'Email issues'],
      [BotIntent.COMPLAINT]: ['Add details', 'Talk to manager', 'Submit feedback'],
      [BotIntent.HUMAN_AGENT_REQUEST]: ['Connect now', 'Browse help first', 'Leave message'],
    };

    return quickReplies[intent] || ['Talk to human', 'Browse help', 'Main menu'];
  }

  private getSuggestedActions(intent: BotIntent): string[] {
    const actions: Record<string, string[]> = {
      [BotIntent.COURSE_INQUIRY]: ['search_courses', 'show_categories'],
      [BotIntent.ENROLLMENT_HELP]: ['check_enrollment_status', 'show_enrollment_guide'],
      ...
      [BotIntent.CERTIFICATE_INQUIRY]: ['check_certificate_eligibility', 'show_progress'],
      [BotIntent.TECHNICAL_SUPPORT]: ['create_support_ticket', 'show_troubleshooting'],
      [BotIntent.ACCOUNT_HELP]: ['navigate_to_profile', 'trigger_password_reset'],
    };
