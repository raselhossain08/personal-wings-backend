...
export class BotActionsService {
  ...
  async getRecommendedCourses(userId: string, limit = 5): Promise<any[]> {
    ...
    const categories = enrollments.map((e: any) => e.course?.category).filter(Boolean);
    
    return this.courseModel
      .find({
        category: { $in: categories },
        isPublished: true,
        _id: { $nin: enrollments.map((e: any) => e.course?._id).filter(Boolean) },
      })
      .limit(limit)
      .sort({ rating: -1 })
      .exec();
  ...
  async checkEnrollmentStatus(userId: string, courseId: string): Promise<any> {
    ...
    const completedLessonsCount = enrollment.completedLessons ? enrollment.completedLessons.size : 0;
  ...
  async canEnrollInCourse(userId: string, courseId: string): Promise<any> {
    const enrollment = await this.enrollmentModel.findOne({ student: userId, course: courseId }).exec();
  ...
  async checkCertificateEligibility(userId: string, courseId: string): Promise<any> {
    const enrollment = await this.enrollmentModel
      .findOne({ student: userId, course: courseId })
      .populate('course')
      .exec();
    ...
    const eligible = requirements.progressComplete && requirements.quizPassed && requirements.assignmentsComplete;
  ...
  private calculateAccountAge(createdAt: Date): string {
    ...
    const months = Math.floor((Date.now() - createdAt.getTime()) / (1000 * 60 * 60 * 24 * 30));
