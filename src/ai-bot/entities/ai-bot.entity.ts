...
export class BotConversation extends Document {
  ...
  @Prop({ type: [{ role: String, content: String, intent: String, confidence: Number, timestamp: Date }], default: [] })
  messages: Array<{
...
export const BotConversationSchema = SchemaFactory.createForClass(BotConversation);
...
export class KnowledgeBase extends Document {
  ...
  @Prop({ type: String, enum: Object.values(ResponseType), default: ResponseType.TEXT })
...
export class BotTask extends Document {
  ...
  @Prop({ type: String, enum: ['pending', 'processing', 'completed', 'failed', 'cancelled'], default: 'pending' })
  status: string;
