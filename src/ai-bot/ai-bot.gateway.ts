...
export class AiBotGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleMessage(@MessageBody() data: any, @ConnectedSocket() client: Socket) {
    ...
    this.server.to(`bot-session-${data.sessionId}`).emit('bot-typing', { isTyping: true });
    ...
    try {
      ...
      this.server.to(`bot-session-${data.sessionId}`).emit('bot-typing', { isTyping: false });

      // Send bot response
      this.server.to(`bot-session-${data.sessionId}`).emit('bot-message', response);
  ...
  async handleQuickReply(@MessageBody() data: any, @ConnectedSocket() client: Socket) {
    const userId = this.activeUsers.get(client.id);
    if (!userId) {
      return { event: 'error', data: { message: 'User not authenticated' } };
    }
