...
export class AiBotService {
  constructor(
    @InjectModel(BotConversation.name) private botConversationModel: Model<BotConversation>,
    @InjectModel(KnowledgeBase.name) private knowledgeBaseModel: Model<KnowledgeBase>,
    @InjectModel(BotAnalytics.name) private botAnalyticsModel: Model<BotAnalytics>,
  ...
  async sendMessage(userId: string, sendMessageDto: SendMessageDto): Promise<any> {
    ...
    const { intent, confidence } = await this.detectIntent(message, conversation);
    ...
    const botResponse = await this.generateResponse(intent, message, conversation, userId);
    ...
    if (botResponse.contextUpdate) {
      conversation.context = { ...conversation.context, ...botResponse.contextUpdate };
    }
  ...
  private async detectIntent(message: string, conversation?: BotConversation): Promise<{ intent: BotIntent; confidence: number }> {
    ...
    if (this.chatGPTService.isEnabled() && conversation) {
      try {
        ...
        console.log('ChatGPT intent detection failed, falling back to keywords');
      }
    }
    ...
    const intentPatterns = [
      { intent: BotIntent.GREETING, keywords: ['hello', 'hi', 'hey', 'good morning', 'good afternoon'], confidence: 0.95 },
      { intent: BotIntent.GOODBYE, keywords: ['bye', 'goodbye', 'see you', 'thanks', 'thank you'], confidence: 0.9 },
      { intent: BotIntent.COURSE_INQUIRY, keywords: ['course', 'learn', 'study', 'class', 'lesson', 'curriculum'], confidence: 0.85 },
      { intent: BotIntent.ENROLLMENT_HELP, keywords: ['enroll', 'register', 'sign up', 'join course', 'start learning'], confidence: 0.85 },
      { intent: BotIntent.PAYMENT_ISSUE, keywords: ['payment', 'pay', 'credit card', 'billing', 'charge', 'invoice'], confidence: 0.9 },
      { intent: BotIntent.TECHNICAL_SUPPORT, keywords: ['error', 'bug', 'not working', 'broken', 'problem', 'issue', 'technical'], confidence: 0.85 },
      { intent: BotIntent.REFUND_REQUEST, keywords: ['refund', 'money back', 'cancel order', 'return'], confidence: 0.95 },
      { intent: BotIntent.CERTIFICATE_INQUIRY, keywords: ['certificate', 'diploma', 'credential', 'completion'], confidence: 0.9 },
      { intent: BotIntent.ACCOUNT_HELP, keywords: ['account', 'password', 'login', 'profile', 'settings'], confidence: 0.85 },
      { intent: BotIntent.COMPLAINT, keywords: ['complaint', 'disappointed', 'angry', 'terrible', 'worst', 'bad'], confidence: 0.9 },
      { intent: BotIntent.HUMAN_AGENT_REQUEST, keywords: ['human', 'agent', 'talk to someone', 'representative', 'person'], confidence: 0.95 },
    ];
  ...
  private async generateResponse(
    ...
    if (this.chatGPTService.isEnabled()) {
      try {
        ...
        console.log('ChatGPT response generation failed, falling back to templates');
      }
    }
    ...
    switch (intent) {
      ...
      case BotIntent.COURSE_INQUIRY:
        ...
        const searchResults = await this.executeAction('search_courses', message, userId, conversation);
        
        if (searchResults && searchResults.length > 0) {
          const courseList = searchResults.map((c, i) => 
            `${i + 1}. ${c.title} - $${c.price}`
          ).join('\n');
        ...
        return {
          ...
          quickReplies: ['Web Development', 'Data Science', 'Business', 'Design', 'Browse all courses'],
          actions: ['search_courses'],
        };

      case BotIntent.ENROLLMENT_HELP:
        // Get user's enrollments
        const enrollments = await this.executeAction('get_my_enrollments', message, userId, conversation);
        ...
        if (enrollments && enrollments.length > 0) {
          return {
            ...
            quickReplies: ['View all enrollments', 'Find courses', 'Check progress'],
      ...
      case BotIntent.PAYMENT_ISSUE:
        ...
        const orders = await this.executeAction('get_my_orders', message, userId, conversation);
        
        if (orders && orders.length > 0) {
          const pendingOrders = orders.filter((o: any) => ['pending', 'processing'].includes(o.status));
          ...
          if (pendingOrders.length > 0) {
            return {
              ...
              quickReplies: ['Retry payment', 'Request refund', 'Talk to human'],
        ...
        return {
          ...
          quickReplies: ['View my orders', 'Payment methods', 'Request refund', 'Talk to human'],
          actions: ['check_payment_status'],
        };

      case BotIntent.TECHNICAL_SUPPORT:
        await this.createSupportTask(conversation._id as any, userId, 'technical_issue', { message });
        return {
          ...
          quickReplies: ['Problem solved', 'Still not working', 'Talk to technician'],
        };
      ...
      case BotIntent.REFUND_REQUEST:
        return {
          ...
          quickReplies: ['Request refund', 'Check refund status', 'Refund policy'],
      ...
      case BotIntent.CERTIFICATE_INQUIRY:
        ...
        if (conversation.context?.currentCourseId) {
          const eligibility = await this.executeAction('check_certificate_eligibility', message, userId, conversation);
          ...
          if (eligibility && !eligibility.error) {
            if (eligibility.eligible) {
              return {
                ...
                quickReplies: ['View certificate', 'Share achievement', 'Continue learning'],
              };
            } else {
              return {
        ...
        return {
          ...
          quickReplies: ['Check progress', 'View certificates', 'Certificate requirements'],
        };

      case BotIntent.ACCOUNT_HELP:
        return {
          message: `I can help with account-related issues:\n\n👤 Profile settings\n🔐 Password reset\n📧 Email verification\n⚙️ Preferences\n\nWhat do you need help with?`,
          quickReplies: ['Reset password', 'Update profile', 'Email issues', 'Talk to human'],
        };
      ...
      case BotIntent.HUMAN_AGENT_REQUEST:
        await this.escalateToHuman(conversation.sessionId, 'user_request', userId);
        return {
  ...
  private async searchKnowledge(query: string, intent: BotIntent): Promise<any> {
  ...
  async getTasks(filters: any): Promise<any> {
    const { page = 1, limit = 20, status, priority, assignedTo, taskType } = filters;
    const query: any = {};
  ...
  async updateTask(taskId: string, updateDto: any): Promise<any> {
    ...
    if (updateDto.assignedTo) task.assignedTo = updateDto.assignedTo as any;
  ...
  async bulkAssignTasks(taskIds: string[], assignedTo: string): Promise<any> {
    const updated = await this.botTaskModel.updateMany(
      { _id: { $in: taskIds } },
  ...
  async getTaskStats(): Promise<any> {
    ...
    const priorityDistribution = await this.botTaskModel.aggregate([
      { $group: { _id: '$priority', count: { $sum: 1 } } },
    ]).exec();

    const taskTypeDistribution = await this.botTaskModel.aggregate([
  ...
  async escalateToHuman(sessionId: string, reason: string, userId: string): Promise<any> {
    const conversation = await this.botConversationModel.findOne({ sessionId }).exec();
    if (!conversation) {
    ...
    await this.createSupportTask(conversation._id as any, userId, 'escalate_to_human', {
      reason,
  ...
  async rateConversation(sessionId: string, rating: number, feedback?: string): Promise<any> {
    const conversation = await this.botConversationModel.findOne({ sessionId }).exec();
    if (!conversation) {
      throw new NotFoundException('Conversation not found');
    }
  ...
  async addKnowledge(createKnowledgeDto: CreateKnowledgeDto): Promise<KnowledgeBase> {
    const knowledge = new this.knowledgeBaseModel(createKnowledgeDto);
    return knowledge.save();
  }

  async updateKnowledge(id: string, updates: Partial<CreateKnowledgeDto>): Promise<KnowledgeBase | null> {
    return this.knowledgeBaseModel.findByIdAndUpdate(id, updates, { new: true }).exec();
  }
  ...
  async getBotAnalytics(startDate?: Date, endDate?: Date): Promise<any> {
    ...
    const analytics = await this.botAnalyticsModel.find(query).sort({ date: -1 }).exec();
    ...
    if (analytics.length > 0) {
      ...
      summary.resolutionRate = (summary.resolvedByBot / summary.totalConversations) * 100;
    }
    ...
    const defaultKnowledge = [
      {
        ...
        answer: 'Creating an account is easy! Click the "Sign Up" button, enter your email, create a password, and verify your email. You\'ll be ready to start learning!',
      ...
      },
      {
        ...
        answer: 'We accept: Credit/Debit Cards (Visa, Mastercard, Amex), PayPal, and Bank Transfers. All payments are secure and encrypted.',
      ...
      },
      {
        ...
        answer: 'Yes! Most courses offer free preview lessons. Look for the "Preview" button on the course page to watch sample content.',
  ...
  private async executeAction(action: string, query: string, userId: string, conversation: BotConversation): Promise<any> {
    try {
      switch (action) {
        case 'search_courses':
        ...
        case 'check_progress':
          ...
          if (courseId) {
            return await this.botActionsService.getEnrollmentProgress(userId, courseId);
          }
          return null;
        ...
        case 'check_certificate_eligibility':
          ...
          if (certCourseId) {
            return await this.botActionsService.checkCertificateEligibility(userId, certCourseId);
          }
          return null;
