...
export class CloudinaryProvider {
  ...
  async uploadFromUrl(
    ...
    try {
      ...
      throw new BadRequestException(`Cloudinary URL upload failed: ${error.message}`);
    }
  }
  ...
  async generateSignedUrl(publicId: string, options: any = {}): Promise<string> {
    return cloudinary.utils.private_download_url(publicId, options.format, options);
  }
  ...
  async getResource(publicId: string): Promise<any> {
    try {
      ...
      throw new BadRequestException(`Cloudinary resource fetch failed: ${error.message}`);
    }
  }
}