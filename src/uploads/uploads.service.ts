...
export class UploadsService {
  ...
  async uploadFile(
    ...
    // Validate file size
    const maxFileSize = this.configService.get<number>('MAX_FILE_SIZE', 5242880);
    ...
    if (!allowedTypes.includes(file.mimetype)) {
      throw new BadRequestException(`File type ${file.mimetype} is not allowed for ${fileType} uploads`);
    }
  ...
  async uploadFromUrl(
    ...
    try {
      ...
      const fileRecord = new this.fileModel({
        ...
        fileName: uploadResult.public_id,
        mimeType: uploadResult.format ? `image/${uploadResult.format}` : 'application/octet-stream',
  ...
  async deleteFile(id: string, userId: string): Promise<void> {
    ...
    try {
      ...
      await this.cloudinaryProvider.deleteFile(file.fileName, this.getResourceType(file.mimeType));
