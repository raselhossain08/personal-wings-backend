...
export class ProductsService {
  ...
  async create(createProductDto: CreateProductDto, sellerId: string): Promise<Product> {
    ...
    const existingProduct = await this.productModel.findOne({ slug: createProductDto.slug });
    if (existingProduct) {
  ...
  async findById(id: string): Promise<Product> {
    ...
    const product = await this.productModel
      .findById(id)
      .populate('seller', 'firstName lastName email phone avatar certifications flightHours')
      .exec();
  ...
  async getSellerProducts(sellerId: string, page: number = 1, limit: number = 10): Promise<{ products: Product[]; total: number }> {
    const skip = (page - 1) * limit;
  ...
  async getProductStats(): Promise<any> {
    ...
    const publishedProducts = await this.productModel.countDocuments({ status: ProductStatus.PUBLISHED });
    const totalAircraft = await this.productModel.countDocuments({ type: ProductType.AIRCRAFT });
    const totalSold = await this.productModel.countDocuments({ status: ProductStatus.SOLD });
  ...
  async searchProducts(query: string, filters: any = {}): Promise<Product[]> {
    ...
    if (filters.minYear) searchQuery.year = { ...searchQuery.year, $gte: filters.minYear };
    if (filters.maxYear) searchQuery.year = { ...searchQuery.year, $lte: filters.maxYear };
