...
export class SecurityController {
  ...
  async getSecurityStatus() {
    const securityEnabled = await this.systemConfigService.getValue('SECURITY_ENABLED', 'false');
  ...
  @Put('toggle')
  @ApiOperation({ summary: 'Enable/Disable security features (requires server restart)' })
  async toggleSecurity(@Body() body: { enabled: boolean }) {
    await this.systemConfigService.update('SECURITY_ENABLED', { value: body.enabled ? 'true' : 'false' });
