...
export class InstructorController {
  ...
  async getDashboard(@CurrentUser() user: any) {
    ...
    const stats = {
      ...
      avgCourseRating: courses.reduce((sum, c) => sum + (c.rating || 0), 0) / courses.length || 0,
      totalRevenue: revenue[0]?.total || 0,
      avgStudentProgress: enrollments.reduce((sum, e) => sum + e.progress, 0) / enrollments.length || 0,
    };
    ...
    const topCourses = await Promise.all(
      courses
        ...
        .slice(0, 5)
        .map(async course => {
          const enrollmentCount = await this.enrollmentModel.countDocuments({ course: course._id });
          return {
            id: course._id,
            title: course.title,
  ...
  async getMyCourses(@CurrentUser() user: any) {
    ...
    const courses = await this.courseModel.find({ instructor: instructorId }).sort({ createdAt: -1 });
  ...
  async getRevenue(@CurrentUser() user: any) {
    ...
    const revenueData = await this.orderModel.aggregate([
      { $match: { courses: { $in: courseIds }, status: { $in: ['completed', 'confirmed'] } } },
      {
        $group: {
  ...
  async getAnalytics(@CurrentUser() user: any) {
    ...
    const completionRates = await Promise.all(
      courses.map(async course => {
        const total = await this.enrollmentModel.countDocuments({ course: course._id });
