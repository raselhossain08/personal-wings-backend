...
export class ProgressController {
  ...
  async getCourseProgress(@Param('courseId') courseId: string, @CurrentUser() user: any) {
    ...
    return {
      ...
      certificateEligible: enrollment.progress >= 100 && enrollment.quizzesPassed >= 1,
  ...
  async getStudentProgress(@Param('studentId') studentId: string) {
    const student = await this.userModel.findById(studentId).select('firstName lastName email');
  ...
  async getCourseStudentsProgress(@Param('courseId') courseId: string) {
    ...
    const stats = {
      ...
      avgProgress: enrollments.reduce((sum, e) => sum + e.progress, 0) / enrollments.length || 0,
    };
  ...
  private getNextLesson(course: any, enrollment: any) {
    ...
    for (const lesson of course.lessons) {
      const isCompleted = enrollment.completedLessons?.get(lesson._id.toString());
      if (!isCompleted) {
        return {
          id: lesson._id,
          title: lesson.title,
