...
export class ReportsController {
  ...
  async getSalesReport(@Query() query: { startDate?: string; endDate?: string; groupBy?: string }) {
    const match: any = {};
  ...
  async getEnrollmentReport(@Query() query: { startDate?: string; endDate?: string; courseId?: string }) {
    const match: any = {};
    ...
    return {
      enrollmentStats,
      topCourses,
      totalEnrollments: enrollmentStats.reduce((sum, item) => sum + item.count, 0),
    };
  }
  ...
  async exportUsers(@Res() res: Response, @Query() query: { role?: string }) {
    ...
    // Generate CSV
    const csvHeader = 'First Name,Last Name,Email,Role,Status,Phone,Joined Date\n';
    const csvRows = users
      .map(
        u =>
  ...
  async exportOrders(@Res() res: Response, @Query() query: { startDate?: string; endDate?: string }) {
    const match: any = {};
    if (query.startDate || query.endDate) {
      match.createdAt = {};
      if (query.startDate) match.createdAt.$gte = new Date(query.startDate);
    ...
    const csvHeader = 'Order Number,Customer,Email,Courses,Total,Status,Payment Method,Date\n';
  ...
  async getInstructorPerformance(@Query() query: { instructorId?: string }) {
    ...
    const instructors = await this.userModel.find(match).select('firstName lastName email');
    
    const performanceData = await Promise.all(
      instructors.map(async instructor => {
        const courses = await this.courseModel.countDocuments({ instructor: instructor._id });
