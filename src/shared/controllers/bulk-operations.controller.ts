...
export class BulkOperationsController {
  ...
  async bulkPublishCourses(@Body() body: { courseIds: string[]; isPublished: boolean }) {
  ...
  async bulkEnrollStudents(@Body() body: { userIds: string[]; courseId: string }) {
    ...
    const result = await this.enrollmentModel.insertMany(enrollments, { ordered: false });
  ...
  async getImportTemplate(@Param('entity') entity: string) {
    ...
    return {
      ...
      entity,
      instructions: 'Download this template, fill it, and upload via bulk import API',
    };
  }
}
