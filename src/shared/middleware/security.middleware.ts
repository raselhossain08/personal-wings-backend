...
export class SecurityMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      console.error(`[SECURITY ALERT] IP: ${clientIp}, Path: ${req.path}`, error.message);
  ...
  private validateHeaders(req: Request) {
    const suspiciousHeaders = ['x-requested-with', 'origin', 'referer'];
  ...
  private scanForThreats(req: Request, ip: string) {
    ...
    if (this.containsMaliciousPattern(payload)) {
      console.error(`[THREAT DETECTED] IP: ${ip}, Payload: ${payload.substring(0, 200)}`);
      throw new HttpException(
    ...
    if (this.containsMongoDBInjection(req.query) || 
  ...
  private containsMongoDBInjection(obj: any): boolean {
    ...
    const mongoOperators = ['$gt', '$gte', '$lt', '$lte', '$ne', '$in', '$nin', '$and', '$or', '$not', '$nor', '$exists', '$type', '$expr', '$jsonSchema', '$mod', '$regex', '$text', '$where'];
