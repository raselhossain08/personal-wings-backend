...
import { UserPoints, PointTransaction, Badge, Achievement, PointActivityType } from './entities/gamification.entity';

@Injectable()
export class GamificationService {
  constructor(
    @InjectModel(UserPoints.name) private userPointsModel: Model<UserPoints>,
    @InjectModel(PointTransaction.name) private transactionModel: Model<PointTransaction>,
  ...
  async awardPoints(userId: string, activityType: PointActivityType, reference?: string, referenceModel?: string): Promise<UserPoints> {
  ...
  async getLeaderboard(limit: number = 10): Promise<UserPoints[]> {
    return this.userPointsModel.find()
