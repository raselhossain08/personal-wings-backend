...
export class CouponsService {
  ...
  async create(data: { code: string; type: CouponType; value: number; expiresAt?: Date; maxUses?: number; minPurchaseAmount?: number }): Promise<Coupon> {
    const coupon = new this.couponModel(data);
    return await coupon.save();
  }

  async validate(code: string, amount: number): Promise<{ valid: boolean; discount: number; coupon?: Coupon }> {
    ...
    const discount = coupon.type === CouponType.PERCENTAGE 
