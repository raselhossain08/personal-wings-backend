...
export class LiveSessionsService {
  ...
  async create(createSessionDto: CreateLiveSessionDto, instructorId: string): Promise<LiveSession> {
  ...
  async findAll(query: {
    ...
    const { courseId, instructorId, status, upcoming, page = 1, limit = 10 } = query;
  ...
  async update(id: string, updateSessionDto: UpdateLiveSessionDto, instructorId: string): Promise<LiveSession> {
  ...
  async joinSession(sessionId: string, userId: string): Promise<LiveSession> {
    ...
    if (session.instructor.toString() === userId && session.status === SessionStatus.SCHEDULED) {
  ...
  async startSession(sessionId: string, instructorId: string): Promise<LiveSession> {
    const session = await this.sessionModel.findById(sessionId);
  ...
  async endSession(sessionId: string, instructorId: string, recordingUrl?: string): Promise<LiveSession> {
    const session = await this.sessionModel.findById(sessionId);
  ...
  async getUpcomingSessions(userId: string, limit: number = 5): Promise<LiveSession[]> {
    const now = new Date();
  ...
  async getSessionStats(sessionId: string): Promise<any> {
    ...
    const attendanceRate = session.maxAttendees > 0
      ? Math.round((attendanceCount / session.maxAttendees) * 100)
      : 0;

    const averageAttendanceTime = session.endedAt && session.startedAt
