...
export class LoginAttempt extends Document {
  ...
  @ApiProperty({ enum: AttemptStatus, example: AttemptStatus.SUCCESS, description: 'Attempt status' })
  ...
  @ApiProperty({ example: 'Invalid password', description: 'Failure reason', required: false })
  @Prop()
  ...
  @ApiProperty({ example: '2fa_required', description: 'Additional context', required: false })
  @Prop()
  context: string;
