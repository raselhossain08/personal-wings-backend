...
export class SecurityLog extends Document {
  ...
  @ApiProperty({ enum: SecurityEventType, example: SecurityEventType.LOGIN, description: 'Event type' })
  @Prop({ type: String, enum: SecurityEventType, required: true })
  eventType: SecurityEventType;

  @ApiProperty({ example: 'User logged in successfully', description: 'Event description' })
  @Prop({ required: true })
  description: string;
  ...
  @ApiProperty({ example: 'device_123', description: 'Device ID', required: false })
  @Prop()
  deviceId: string;
  ...
  @ApiProperty({ example: 'Invalid credentials', description: 'Failure reason', required: false })
  @Prop()
