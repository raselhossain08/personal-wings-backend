...
export class AttendanceService {
  ...
  async markAttendance(markAttendanceDto: MarkAttendanceDto, userId: string, req: any): Promise<Attendance> {
    const session = await this.liveSessionModel.findById(markAttendanceDto.sessionId).exec();
    if (!session) {
  ...
  async getSessionAttendance(sessionId: string): Promise<any> {
    ...
    const averageDuration = attendances.reduce((sum, a) => sum + a.duration, 0) / (totalAttendees || 1);
    ...
    return {
      ...
      stats: {
        ...
        attendanceRate: totalAttendees > 0 ? (presentCount / totalAttendees) * 100 : 0,
  ...
  async getAttendanceReport(filters: any): Promise<any> {
    ...
    const presentCount = attendances.filter(a => a.present).length;
    const uniqueUsers = new Set(attendances.map(a => a.userId.toString())).size;
    const uniqueSessions = new Set(attendances.map(a => a.sessionId.toString())).size;
    const totalDuration = attendances.reduce((sum, a) => sum + a.duration, 0);
  ...
  async updateCertificateEligibility(attendanceId: string, eligible: boolean): Promise<Attendance | null> {
