...
export class WishlistService {
  ...
  async removeFromWishlist(userId: string, courseId: string): Promise<Wishlist | null> {
  ...
  async getWishlist(userId: string): Promise<Wishlist | null> {
    return this.wishlistModel.findOne({ user: userId }).populate('courses').exec();
  }

  async addToCart(userId: string, itemId: string, itemType: 'Course' | 'Product', price: number, quantity: number = 1): Promise<Cart> {
    ...
    if (existingItem) {
      ...
      cart.items.push({ itemId: new Types.ObjectId(itemId), itemType, price, quantity } as any);
    }
