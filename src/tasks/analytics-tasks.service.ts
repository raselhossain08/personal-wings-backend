...
export class AnalyticsTasksService {
  ...
  async aggregateDailyAnalytics() {
    ...
    try {
      ...
      const [pageViews, uniqueVisitors, conversions, revenue] = await Promise.all([
  ...
  private async aggregateUniqueVisitors(start: Date, end: Date): Promise<number> {
  ...
  private async getWeeklyStats(startDate: Date): Promise<any> {
    ...
    const [pageViews, uniqueVisitors, conversions, topPages, trafficSources] = await Promise.all([
