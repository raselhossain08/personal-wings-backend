...
export class AnalyticsService {
  ...
  async getRevenueAnalytics(period: AnalyticsPeriod = AnalyticsPeriod.MONTH) {
    ...
    const revenueData = await this.ordersService.getRevenueByDateRange(dateRange);
  ...
  async getEnrollmentAnalytics(period: AnalyticsPeriod = AnalyticsPeriod.MONTH) {
    const dateRange = this.getDateRange(period);
    
    const enrollmentData = await this.coursesService.getEnrollmentsByDateRange(dateRange);
    const chartData = this.formatChartData(enrollmentData, period);

    const totalEnrollments = enrollmentData.reduce((sum, item) => sum + item.count, 0);
  ...
  async getInstructorPerformance() {
    ...
    const performanceData = await Promise.all(
      instructors.map(async (instructor) => {
        const courses = await this.coursesService.getInstructorCourses(instructor.id);
        const revenue = courses.courses.reduce((sum, course) => sum + course.totalRevenue, 0);
        const enrollments = courses.courses.reduce((sum, course) => sum + course.studentCount, 0);
        const averageRating = courses.courses.reduce((sum, course) => sum + course.rating, 0) / courses.courses.length;
        ...
        return {
          ...
          metrics: {
            ...
            completionRate: await this.getInstructorCompletionRate(instructor.id),
          },
    ...
    return performanceData.sort((a, b) => b.metrics.totalRevenue - a.metrics.totalRevenue);
  }

  async getGeographicDistribution() {
    const distribution = await this.usersService.getGeographicDistribution();
    const enrollmentByCountry = await this.ordersService.getEnrollmentsByCountry();
  ...
  async getConversionRates() {
    ...
    const enrollmentToPurchase = (purchaseData.length / enrollmentData.length) * 100;
    ...
    return {
      ...
      enrollmentToPurchase: isNaN(enrollmentToPurchase) ? 0 : enrollmentToPurchase,
  ...
  private formatChartData(data: any[], period: AnalyticsPeriod): any[] {
    ...
    const chartData = Array.from({ length: this.getPeriodLength(period) }, (_, i) => ({
  ...
  private async getRevenueGrowth(period: AnalyticsPeriod): Promise<number> {
    ...
    const currentTotal = currentRevenue.reduce((sum, item) => sum + item.amount, 0);
    const previousTotal = previousRevenue.reduce((sum, item) => sum + item.amount, 0);
  ...
  private async getInstructorCompletionRate(instructorId: string): Promise<number> {
  ...
  private async getEnrollmentEvents(): Promise<any[]> {
    return await this.analyticsModel.find({ eventType: 'course_enrollment' }).exec();
  }
