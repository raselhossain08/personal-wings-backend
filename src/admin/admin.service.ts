...
export class AdminService {
  ...
  async getTopCourses(limit = 10): Promise<any> {
    ...
    return topCourses.map((item) => ({
      ...
      completionRate: item.enrollmentCount > 0
        ? parseFloat(((item.completionCount / item.enrollmentCount) * 100).toFixed(2))
        : 0,
    }));
  }

  async getTopInstructors(limit = 10): Promise<any> {
    const topInstructors = await this.courseModel.aggregate([
      {
        $lookup: {
          from: 'enrollments',
          localField: '_id',
  ...
  async bulkDeleteUsers(userIds: string[]): Promise<any> {
    const result = await this.userModel.deleteMany({ _id: { $in: userIds } }).exec();
  ...
  async getInstructorStats(instructorId: string): Promise<any> {
    const [courses, totalEnrollments, totalRevenue, avgRating] = await Promise.all([
      this.courseModel.find({ instructor: instructorId }).exec(),
      this.enrollmentModel.countDocuments({ 
        course: { $in: await this.courseModel.find({ instructor: instructorId }).distinct('_id') }
      }).exec(),
      this.orderModel.aggregate([
        {
          $match: {
      ...
      this.reviewModel.aggregate([
        {
          $match: {
            itemId: { $in: await this.courseModel.find({ instructor: instructorId }).distinct('_id') },
          },
        },
  ...
  async getFlaggedContent(page = 1, limit = 20): Promise<any> {
    ...
    return {
      ...
      pagination: {
        ...
        page,
        limit,
        totalPages: Math.ceil(await this.reviewModel.countDocuments({ flagged: true }).exec() / limit),
      },
    };
  }

  async flagContent(contentId: string, contentType: string, reason: string): Promise<any> {
  ...
  async getPlatformStats(): Promise<any> {
    ...
    const [
      ...
      this.courseModel.countDocuments().exec(),
      this.courseModel.countDocuments({ createdAt: { $gte: thirtyDaysAgo } }).exec(),
      ...
      this.enrollmentModel.countDocuments({ completedAt: { $exists: true } }).exec(),
    ]);
    ...
    return {
      ...
      enrollments: {
        ...
        completionRate: activeEnrollments > 0 
          ? parseFloat(((completedEnrollments / activeEnrollments) * 100).toFixed(2))
          : 0,
      },
    };
  }
  ...
  async getRecentActivity(limit = 50): Promise<any> {
    const [recentUsers, recentCourses, recentOrders, recentEnrollments] = await Promise.all([
      this.userModel
        .find()
