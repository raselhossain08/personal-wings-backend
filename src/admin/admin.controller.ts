...
export class AdminController {
  ...
  getAllCourses(
    ...
    return this.adminService.getAllCourses({ page, limit, status, category, search });
  }
  ...
  getAllOrders(
    ...
    return this.adminService.getAllOrders({ page, limit, status, paymentStatus });
  }
  ...
  getTopCourses(@Query('limit') limit?: number) {
    return this.adminService.getTopCourses(limit ? parseInt(limit.toString()) : 10);
  }

  @Get('analytics/top-instructors')
  getTopInstructors(@Query('limit') limit?: number) {
    return this.adminService.getTopInstructors(limit ? parseInt(limit.toString()) : 10);
  }
  ...
  getPendingInstructors(@Query('page') page?: number, @Query('limit') limit?: number) {
  ...
  // ==================== CONTENT MODERATION ====================
  @Get('content/flagged')
  getFlaggedContent(@Query('page') page?: number, @Query('limit') limit?: number) {
  ...
  unflagContent(@Param('id') id: string, @Body('contentType') contentType: string) {
  ...
  searchAll(@Query('q') query: string, @Query('limit') limit?: number) {
    return this.adminService.searchAll(query, limit ? parseInt(limit.toString()) : 10);
  }
  ...
  exportOrders(
    ...
    return this.adminService.exportOrders(new Date(startDate), new Date(endDate));
  }
}
