...
export class CampaignsService {
  ...
  async trackEvent(trackEventDto: TrackEventDto, req: any): Promise<CampaignEvent> {
    const { campaignId, eventType, value, metadata, ...eventData } = trackEventDto;
  ...
  async getAnalytics(id: string, analyticsDto: CampaignAnalyticsDto): Promise<any> {
    ...
    const roi = campaign.spent > 0 ? ((metrics.revenue - campaign.spent) / campaign.spent) * 100 : 0;
    const cpc = metrics.clicks > 0 ? campaign.spent / metrics.clicks : 0;
    const cpa = metrics.conversions > 0 ? campaign.spent / metrics.conversions : 0;
  ...
  async getTopCampaigns(limit: number = 10): Promise<any[]> {
    ...
    return campaigns.map((campaign) => ({
      ...
      roi: campaign.spent > 0 ? ((campaign.revenue - campaign.spent) / campaign.spent) * 100 : 0,
  ...
  private getDateRange(analyticsDto: CampaignAnalyticsDto): { startDate: Date; endDate: Date } {
    const now = new Date();
    let startDate: Date;
