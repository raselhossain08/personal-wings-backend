...
export class OrdersService {
  ...
  async updateStatus(id: string, status: OrderStatus, reason?: string): Promise<Order> {
    ...
    if (status === OrderStatus.COMPLETED) {
      ...
      await this.notificationsService.sendPaymentSuccessNotification(user.id, order);
    }
  ...
  async getRevenueByDateRange(dateRange: { start: Date; end: Date }): Promise<any[]> {
  ...
  async countCompletedOrders(): Promise<number> {
    return await this.orderModel.countDocuments({ status: OrderStatus.COMPLETED });
  }
