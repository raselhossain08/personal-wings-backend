...
export class RefundsService {
  ...
  async createRefund(createRefundDto: CreateRefundDto, userId: string): Promise<Refund> {
    // Verify order exists and belongs to user
    const order = await this.orderModel.findById(createRefundDto.orderId).exec();
    if (!order) {
    ...
    if (daysSinceOrder > 30) {
      throw new BadRequestException('Refund requests must be made within 30 days of purchase');
    }
  ...
  async reviewRefund(id: string, reviewRefundDto: ReviewRefundDto, adminId: string): Promise<Refund | null> {
    ...
    if (reviewRefundDto.reviewNotes) refund.reviewNotes = reviewRefundDto.reviewNotes;
    if (reviewRefundDto.rejectionReason) refund.rejectionReason = reviewRefundDto.rejectionReason;
  ...
  async getRefundStats(): Promise<any> {
    const [total, pending, approved, rejected, completed] = await Promise.all([
      ...
      this.refundModel.countDocuments({ status: RefundStatus.COMPLETED }).exec(),
    ]);

    const totalRefunded = await this.refundModel.aggregate([
