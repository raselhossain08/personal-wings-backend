...
import { QuizSubmission, SubmissionStatus, QuizAnswer } from './entities/quiz-submission.entity';
...
export class QuizzesService {
  ...
  async create(createQuizDto: CreateQuizDto, instructorId: string): Promise<Quiz> {
  ...
  async update(id: string, updateQuizDto: UpdateQuizDto, instructorId: string): Promise<Quiz> {
  ...
  private checkAnswer(userAnswer: string | string[], correctAnswer: any): boolean {
  ...
  async getSubmission(submissionId: string, userId: string): Promise<QuizSubmission> {
  ...
  async getQuizSubmissions(
    ...
    const [submissions, total, stats] = await Promise.all([
      this.submissionModel
        .find({ quiz: quizId, status: { $in: [SubmissionStatus.SUBMITTED, SubmissionStatus.GRADED] } })
        .populate('student', 'firstName lastName email')
        .sort({ submittedAt: -1 })
        .skip(skip)
        .limit(limit)
        .exec(),
      this.submissionModel.countDocuments({ quiz: quizId, status: { $in: [SubmissionStatus.SUBMITTED, SubmissionStatus.GRADED] } }),
