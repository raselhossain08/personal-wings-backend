...
export class NotificationsService {
  ...
  async sendNotification(notificationData: {
    ...
    const userId = typeof notificationData.user === 'string' 
    ...
    try {
      switch (notificationData.type) {
        ...
        default:
          this.logger.warn(`Unsupported notification type: ${notificationData.type}`);
      }
  ...
  async sendBulkNotifications(notificationsData: Partial<Notification>[]): Promise<{ success: number; failed: number }> {
  ...
  async sendWelcomeNotification(userId: string): Promise<Notification> {
    return this.sendNotification({
      ...
      message: 'Thank you for joining our aviation training platform. We\'re excited to help you advance your career.',
      template: 'welcome',
      priority: 'high',
    });
  }

  async sendCourseEnrollmentNotification(userId: string, course: any): Promise<Notification> {
    return this.sendNotification({
      user: userId,
      type: NotificationType.EMAIL,
  ...
  async sendPaymentSuccessNotification(userId: string, order: any): Promise<Notification> {
    return this.sendNotification({
      user: userId,
      type: NotificationType.EMAIL,
  ...
  async sendCertificateEarnedNotification(userId: string, course: any, certificateUrl: string): Promise<Notification> {
    return this.sendNotification({
      user: userId,
      type: NotificationType.EMAIL,
