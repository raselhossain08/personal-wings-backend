...
export class CoursesService {
  ...
  async findById(id: string): Promise<Course> {
    ...
    const course = await this.courseModel
      .findById(id)
      .populate('instructor', 'firstName lastName email avatar bio certifications flightHours')
      .exec();
  ...
  async update(id: string, updateCourseDto: UpdateCourseDto, userId: string, userRole: UserRole): Promise<Course> {
  ...
  async getInstructorCourses(instructorId: string, page: number = 1, limit: number = 10): Promise<{ courses: Course[], total: number }> {
    return this.findAll(page, limit, undefined, undefined, undefined, instructorId);
  }
  ...
  async createLesson(courseId: string, createLessonDto: CreateLessonDto, instructorId: string): Promise<Lesson> {
    ...
    if (existingLesson) {
      throw new ConflictException('Lesson with this slug already exists in this course');
    }
  ...
  async getCourseLessons(courseId: string, userId?: string, userRole?: UserRole): Promise<Lesson[]> {
    const course = await this.findById(courseId);
  ...
  async updateLesson(lessonId: string, updateData: any, userId: string, userRole: UserRole): Promise<Lesson> {
  ...
  async getStats(): Promise<any> {
    ...
    const publishedCourses = await this.courseModel.countDocuments({ status: CourseStatus.PUBLISHED });
  ...
  async incrementEnrollment(courseId: string, amount: number = 1): Promise<void> {
  ...
  async getEnrollmentsByDateRange(dateRange: { start: Date; end: Date }): Promise<any[]> {
