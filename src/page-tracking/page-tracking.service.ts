...
import { PageView, UserSession, UserEvent, Heatmap } from './entities/page-tracking.entity';
...
export class PageTrackingService {
  ...
  // Track page view
  async trackPageView(trackPageViewDto: TrackPageViewDto, req: any): Promise<any> {
  ...
  async trackEvent(trackEventDto: TrackEventDto, req: any): Promise<UserEvent> {
    ...
    if (trackEventDto.eventType === 'click' && trackEventDto.positionX && trackEventDto.positionY) {
  ...
  // Update heatmap data
  private async updateHeatmap(page: string, eventType: string, x: number, y: number): Promise<void> {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const heatmapPoint = await this.heatmapModel.findOne({
      page,
      eventType,
      x: { $gte: x - 10, $lte: x + 10 },
      y: { $gte: y - 10, $lte: y + 10 },
  ...
  async getPageAnalytics(query: PageAnalyticsQueryDto): Promise<any> {
    ...
    const analytics = {
      ...
      uniqueVisitors: new Set(pageViews.map(pv => pv.userId?.toString()).filter(Boolean)).size,
      avgTimeOnPage: 0,
      avgScrollDepth: 0,
      bounceRate: 0,
      conversionRate: 0,
    ...
    pageViews.forEach((pv) => {
      ...
      // Device breakdown
      if (pv.deviceType) {
        analytics.deviceBreakdown[pv.deviceType] = (analytics.deviceBreakdown[pv.deviceType] || 0) + 1;
      }

      // Browser breakdown
      if (pv.browser) {
        analytics.browserBreakdown[pv.browser] = (analytics.browserBreakdown[pv.browser] || 0) + 1;
      }

      // Country breakdown
      if (pv.country) {
        analytics.countryBreakdown[pv.country] = (analytics.countryBreakdown[pv.country] || 0) + 1;
      }

      // Daily page views
      const pvDate = (pv as any).createdAt || new Date();
      const dateKey = pvDate.toISOString().split('T')[0];
      analytics.dailyPageViews[dateKey] = (analytics.dailyPageViews[dateKey] || 0) + 1;
    });

    analytics.avgTimeOnPage = pageViews.length > 0 ? Math.round(totalTimeOnPage / pageViews.length) : 0;
    analytics.avgScrollDepth = pageViews.length > 0 ? Math.round(totalScrollDepth / pageViews.length) : 0;
    analytics.bounceRate = pageViews.length > 0 ? parseFloat(((bounces / pageViews.length) * 100).toFixed(2)) : 0;
    analytics.conversionRate = pageViews.length > 0 ? parseFloat(((conversions / pageViews.length) * 100).toFixed(2)) : 0;
  ...
  async getSessionDetails(sessionId: string): Promise<any> {
    ...
    const pageViews = await this.pageViewModel.find({ sessionId }).sort({ createdAt: 1 }).exec();
    const events = await this.userEventModel.find({ sessionId }).sort({ createdAt: 1 }).exec();
  ...
  // Get heatmap data
  async getHeatmapData(page: string, eventType: string = 'click'): Promise<Heatmap[]> {
  ...
  async getConversionFunnel(steps: string[]): Promise<any> {
    ...
    for (const step of steps) {
      const count = await this.pageViewModel.countDocuments({ page: step }).exec();
