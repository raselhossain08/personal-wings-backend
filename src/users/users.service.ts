...
export class UsersService {
  ...
  async findAll(page: number = 1, limit: number = 10, search?: string, role?: UserRole): Promise<{ users: User[], total: number }> {
  ...
  async getStats(): Promise<any> {
    ...
    const instructors = await this.userModel.countDocuments({ role: UserRole.INSTRUCTOR });
